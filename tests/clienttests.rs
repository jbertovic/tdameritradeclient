...
fn able_to_retrieve_userprincipals() {
    let resptxt: String = initialize_client().get(&Endpoint::UserPrincipals, &[param::Empty]).unwrap();
    println!("{:?}", resptxt);
...
fn able_to_retrieve_userprincipals_into_model() {
    assert!(serde_json::from_value::<UserPrincipals>(initialize_client().get(&Endpoint::UserPrincipals, &[param::Empty]).unwrap()).is_ok());
}

#[test]
fn able_to_retrieve_quotes() {
    let resptxt: String =
        initialize_client().get(&Endpoint::Quotes, &[param::Quotes::Symbol("F,VFIAX,SPY,$SPX.X")]).unwrap();
...
fn able_to_retrieve_quotes_into_model() {
    ...
    assert!(serde_json::from_value::<HashMap<String, Quote>>(initialize_client().get(&Endpoint::Quotes, &[param::Quotes::Symbol(quote_symbols)]).unwrap()).is_ok());
}

#[test]
fn able_to_retrieve_tojson() {
    let resptxt: serde_json::Value =
        initialize_client().get(&Endpoint::UserPrincipals, &[param::Empty]).unwrap();
    println!("{:?}", resptxt);
...
fn able_to_retrieve_all_accounts() {
    let resptxt: String = initialize_client().get(&Endpoint::Accounts, &[param::Empty]).unwrap();
    println!("{:?}", resptxt);
...
fn able_to_retrieve_one_account() {
    ...
    let resptxt: String = c.get(&Endpoint::Account(&accountid), &[param::Empty]).unwrap();
    println!("{:?}", resptxt);
    assert_eq!(resptxt.contains("\"securitiesAccount\""), true);
}
...
fn able_to_retrieve_account_into_model() {
    let (c, accountid) = initialize_client_accountid();
    assert!(serde_json::from_value::<AccountRoot>(c.get(&Endpoint::Account(&accountid), &[param::Empty]).unwrap()).is_ok());
    assert!(serde_json::from_value::<AccountRoot>(c.get(&Endpoint::Account(&accountid), &[param::Account::Positions]).unwrap()).is_ok());
    assert!(serde_json::from_value::<AccountRoot>(c.get(&Endpoint::Account(&accountid), &[param::Account::Orders]).unwrap()).is_ok());
}
...
fn able_to_retrieve_account_positions() {
    let (c, accountid) = initialize_client_accountid();
    let resptxt: String = c.get(&Endpoint::Account(&accountid), &[param::Account::Positions]).unwrap();
    println!("{:?}", resptxt);
...
fn able_to_retrieve_transactions() {
    let (c, accountid) = initialize_client_accountid();
    let resptxt: String = c.get(&Endpoint::Transactions(&accountid), &[param::Empty]).unwrap();
    println!("{:?}", resptxt);
...
fn able_to_retrieve_instrument_cusip() {
    let c = initialize_client();
    let resptxt: String = c.get(&Endpoint::Instrument("458140100"), &[param::Empty]).unwrap();
    println!("{:?}", resptxt);
